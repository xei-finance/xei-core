...
contract QuoterV2 is IQuoterV2, IXeiV3SwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for IXeiV3Pool;
    ...
    function getPool(address tokenA, address tokenB, uint24 fee) private view returns (IXeiV3Pool) {
        return IXeiV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }

    /// @inheritdoc IXeiV3SwapCallback
    function xeiV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes memory path) external view override {
        ...
        IXeiV3Pool pool = getPool(tokenIn, tokenOut, fee);
        ...
        returns (uint256 amountOut, uint160 sqrtPriceX96After, uint32 initializedTicksCrossed, uint256 gasEstimate)
    {
        ...
        IXeiV3Pool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
