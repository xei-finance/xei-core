...
import { XeiV3PoolSwapTest } from '../typechain/XeiV3PoolSwapTest'
...
describe('XeiV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('XeiV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as XeiV3PoolSwapTest
