...
import { TestXeiV3SwapPay } from '../typechain/TestXeiV3SwapPay'
...
import { TestXeiV3ReentrantCallee } from '../typechain/TestXeiV3ReentrantCallee'
...
describe('XeiV3Pool', () => {
  ...
  let factory: XeiV3Factory
  let pool: MockTimeXeiV3Pool

  let swapTarget: TestXeiV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeXeiV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Xei/xei-v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestXeiV3ReentrantCallee')
      ).deploy()) as TestXeiV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestXeiV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestXeiV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestXeiV3SwapPay
